...
# Source: Chapter-12/HIcalculation/EXE1_calcHI_performBC_CORDEX_NAM-22.ipynb
...
all_models['rcp85'] = []
all_models['rcp26'] = []
with open(dir_names + 'Models_CORDEX-NAM-22_RCP26.txt', 'r') as filehandle:
    for line in filehandle:
        all_models['rcp26'].append(eval(line[:-1]))
with open(dir_names + 'Models_CORDEX-NAM-22_RCP85.txt', 'r') as filehandle:
    for line in filehandle:
...
regrid_name = CORDEX_reg + '_rlat-REMO'
...
#Define model string
model = ['MPI-M-MPI-ESM-LR', 'OURANOS-CRCM5', 'rlat-OURANOS']
COR_MOD_str = CORDEX_str + '_' + model[0] + '_' + model[1]
...
#Regrid WFDE5 data to model grid
regrid_name = CORDEX_reg + '_rlat-OURANOS'
...
for RCP in RCPs:
    ...
    for model in models:
        ...
        #Select WFDE5 grid
        if model[1]=='OURANOS-CRCM5':      regrid_name = CORDEX_reg + '_rlat-OURANOS'
