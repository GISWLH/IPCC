...
# Source: Chapter-12/HIcalculation/EXE1_calcHI_performBC_CORDEX_WAS-44.ipynb
...
all_models['rcp85'] = []
all_models['rcp26'] = []
with open(dir_names + 'Models_CORDEX-WAS-44_RCP26.txt', 'r') as filehandle:
    for line in filehandle:
        all_models['rcp26'].append(eval(line[:-1]))
with open(dir_names + 'Models_CORDEX-WAS-44_RCP85.txt', 'r') as filehandle:
    for line in filehandle:
...
#Define model string
model = ['MPI-M-MPI-ESM-LR', 'SMHI-RCA4', 'rlat-SMHI_REMO']
...
#Define model string
model = ['MPI-M-MPI-ESM-MR', 'IITM-RegCM4-4', 'rlat-RegCM4']
COR_MOD_str = CORDEX_str + '_' + model[0] + '_' + model[1]
...
for RCP in RCPs:
    ...
    for model in models:
        ...
        #Select WFDE5 grid
        if model[1]=='IITM-RegCM4-4':  regrid_name = CORDEX_reg + '_rlat-RegCM4'
        else:                          regrid_name = CORDEX_reg + '_rlat-SMHI_REMO'
...
#Remove WFDE5 data
my_fun.remove_directories_CORDEX(dir_CORDEX_reg, model_str, COR_MOD_str, dir_data, dir_HIday, CORDEX_reg + '_rlat-SMHI_REMO', True)
