...
# Source: Chapter-12/HIcalculation/EXE1_calcHI_performBC_CMIP6.ipynb
...
dir_CMIP    = '/div/amoc/archive/tmp_clemens/CMIP6/'
...
dir_GWL     = '/div/amoc/exhaustion/Heat_Health_Global/Data/IPCC_AR6/Warming_levels_CMIP5_CMIP6/warming_levels/cmip6_all_ens/'
...
#Define models which should be used
all_models = dict()
all_models['ssp585'] = []
all_models['ssp126'] = []
with open(dir_names + 'Models_CMIP6_SSP126.txt', 'r') as filehandle:
    for line in filehandle:
        all_models['ssp126'].append(line[:-1])
with open(dir_names + 'Models_CMIP6_SSP585.txt', 'r') as filehandle:
    for line in filehandle:
        all_models['ssp585'].append(line[:-1])

#Define heat indices, SSPs and BC method
...
dir_vers = '/div/amoc/exhaustion/Heat_Health_Global/Data/IPCC_AR6/Model_versions/CMIP6/'
...
for SSP in SSPs:
    ...
    for model in models:
        ...
        calendar = my_fun.concat_CMIP(model, var_names_ORIG, dir_data, SSP, member)
        ...
        for heat_ind in heat_indices:
            ...
            #Calculate HSI
            getHI.GET_heat_indices_v2(model, SSP, member, heat_ind, dir_CMIP_in, dir_CMIP_out, vec_orig, dyear_orig, var_names_CORR)
            ...
            for time_sel in time_app:
                ...
                fname_MOD = dir_MOD + heat_ind + "_" + model + "_" + SSP + "_" + member + "_" + str(time_orig[0]) + "-" + str(time_orig[1]) + ".nc"
                fname_out = dir_out + heat_ind + "_" + model + "_" + SSP + "_" + member + "_" + str(time_sel[0]) + "-" + str(time_sel[1]) + ".nc"
                ...
                getMEAN_THRES.calc_thresh_IPCC(model, SSP, dir_HIday, dir_save, heat_ind, BC_methods, [time_sel], member, CMIPvers)
