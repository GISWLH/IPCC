...
# Source: Chapter-12/HIcalculation/EXE1_calcHI_performBC_CORDEX_AFR-22.ipynb
...
all_models['rcp85'] = []
all_models['rcp26'] = []
with open(dir_names + 'Models_CORDEX-AFR-22_RCP26.txt', 'r') as filehandle:
    for line in filehandle:
        all_models['rcp26'].append(eval(line[:-1]))
with open(dir_names + 'Models_CORDEX-AFR-22_RCP85.txt', 'r') as filehandle:
    for line in filehandle:
...
#Define model string
model = ['MPI-M-MPI-ESM-LR', 'GERICS-REMO2015', 'rlat-REMO']
...
regrid_name = CORDEX_reg + '_' + model[2]
...
#Define model string
model = ['MPI-M-MPI-ESM-LR', 'CLMcom-KIT-CCLM5-0-15', 'rlat-CCLM']
COR_MOD_str = CORDEX_str + '_' + model[0] + '_' + model[1]
...
#Define model string
model = ['MPI-M-MPI-ESM-MR', 'ICTP-RegCM4-7', 'rlat-RegCM4']
COR_MOD_str = CORDEX_str + '_' + model[0] + '_' + model[1]
...
#Define model string
model = ['CCCma-CanESM2', 'CCCma-CanRCM4', 'rlat-CanRCM4']
COR_MOD_str = CORDEX_str + '_' + model[0] + '_' + model[1]
...
for RCP in RCPs:
    ...
    for model in models:
        ...
        #Differentiate between psl (for HadGEM-CCLM5) and ps (only for AFR-22!)
        if RCP=='rcp85' and model[0]=='MOHC-HadGEM2-ES' and model[1]=='CLMcom-KIT-CCLM5-0-15':
        ...
        #Select WFDE5 grid
        if model[1]=='GERICS-REMO2015':          regrid_name = CORDEX_reg + '_rlat-REMO'
        elif model[1]=='CLMcom-KIT-CCLM5-0-15':  regrid_name = CORDEX_reg + '_rlat-CCLM'
        elif model[1]=='ICTP-RegCM4-7':          regrid_name = CORDEX_reg + '_rlat-RegCM4'
        elif model[1]=='CCCma-CanRCM4':          regrid_name = CORDEX_reg + '_rlat-CanRCM4'
...
#Remove WFDE5 data
my_fun.remove_directories_CORDEX(dir_CORDEX_reg, model_str, COR_MOD_str, dir_data, dir_HIday, CORDEX_reg + '_rlat-REMO', True)
my_fun.remove_directories_CORDEX(dir_CORDEX_reg, model_str, COR_MOD_str, dir_data, dir_HIday, CORDEX_reg + '_rlat-CCLM', True)
my_fun.remove_directories_CORDEX(dir_CORDEX_reg, model_str, COR_MOD_str, dir_data, dir_HIday, CORDEX_reg + '_rlat-RegCM4', True)
my_fun.remove_directories_CORDEX(dir_CORDEX_reg, model_str, COR_MOD_str, dir_data, dir_HIday, CORDEX_reg + '_rlat-CanRCM4', True)
