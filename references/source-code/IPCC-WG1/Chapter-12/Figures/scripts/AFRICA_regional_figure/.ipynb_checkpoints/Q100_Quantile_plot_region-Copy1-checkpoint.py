...
# Source: Chapter-12/Figures/scripts/AFRICA_regional_figure/.ipynb_checkpoints/Q100_Quantile_plot_region-Copy1-checkpoint.ipynb
...
figname = paste('/home/jservon/Chapter12_IPCC/figs/Figure_12.5/',region,'_Q100_boxplot.png',sep='')
png(figname, width=900,height=1400)
...
for (subregion in continent_regions){
    mar = c(1,1,2.5,1)
    if (subregion %in% c('WSAF','MED','CAF')){
        do_ylab_raw='TRUE'
        mar[2]=4
    }else{
        do_ylab_raw='FALSE'
    }#
    if (subregion %in% c('WAF','SEAF','MDG')){
        do_ylab_diff='TRUE'
        mar[4]=4
    }else{
        do_ylab_diff='FALSE'
    }#
    if (subregion %in% c('WSAF','ESAF','MDG')){
        do_xlab='TRUE'
        mar[1] = 5 # -- bottom margin
    }else{
        do_xlab='FALSE'
