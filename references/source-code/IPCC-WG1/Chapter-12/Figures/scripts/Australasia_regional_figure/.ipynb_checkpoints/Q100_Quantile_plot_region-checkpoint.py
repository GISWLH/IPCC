...
# Source: Chapter-12/Figures/scripts/Australasia_regional_figure/.ipynb_checkpoints/Q100_Quantile_plot_region-checkpoint.ipynb
...
for (continent in names(regions_by_continent)){
    ...
    metrics_filename = paste(CWD,'/../../data/Figure_12.7/Q100_',ensemble,'/Q100_',wclim_period,'.nc_',wcontinent,'.txt',sep='')
...
for (continent in names(regions_by_continent)){
    ...
    metrics_filename = paste(CWD,'/../../data/Figure_12.7/Q100_',wensemble,'/Q100_',wclim_period,'.nc_',wcontinent,'.txt',sep='')
    print(paste('Raw values :', metrics_filename))
    res_metrics_by_AR6region = metrics_reader(metrics_filename)
    for (region_name in names(res_metrics_by_AR6region)){
        all_regions[[region_name]][[ensemble]][[clim_period]][['vals']] = res_metrics_by_AR6region[[region_name]]
...
figname = paste(CWD,'/../../figs/Figure_12.7/panel_c_',region,'_Q100_boxplot.png',sep='')
...
for (subregion in continent_regions){
    ...
    if (subregion %in% c('CAU','SAU')){
        mar[4]=4
    }
    if (subregion %in% c('CAU','SAU','NZ')){
        do_ylab_diff='TRUE'
        #mar[4]=4
    }else{
        do_ylab_diff='FALSE'
    }#
    if (subregion=='NZ'){mar[1] = 5.5 }
    ...
    ylims = list(
        ylim_raw = c(0,1.25),
        ylim_raw_levels = seq(0,1,by=0.5),
        ylim_diff = c(-0.2,0.5),
        ylim_diff_levels = c(-0.2,-0.1,0,0.1,0.2,0.3,0.4)
    )           
...
# %% cell 24
figname = paste(CWD,'/../../figs/Figure_12.7/panel_c_',region,'_Q100_boxplot.pdf',sep='')
