...
# Source: Chapter-12/Figures/scripts/Australasia_regional_figure/.ipynb_checkpoints/CoastalRecession_map_AR6regions_Australasia-checkpoint.ipynb
...
def add_AR6regions(wks, base_map, regions):
    ...
    adjp = dict(
        NAU = dict(lon=0, lat=-2),
        CAU = dict(lon=0, lat=2),
        SAU = dict(lon=0, lat=-2),
    )
...
scenario = 'RCP85'
horizon = '2100'
region_dict = dict(name='Australasia',
                    lonmin = -255,
                    lonmax = -175,
                    latmin = -57,
                    latmax = 0,
...
# Draw a text string labeling the marker
Ngl.text_ndc(wks,"Shoreline position change by "+horizon,0.5,0.77,txres)
Ngl.text_ndc(wks,"CMIP5 RCP8.5",0.5,0.74,txres)
...
trim_figure = '/home/jservon/Chapter12_IPCC/figs/Figure_12.7/panel_b_'+region_dict['name']+'_'+variable+'_map_'+scenario+'_'+horizon+'.pdf'
...
def extract_plot(figure_file,trim_figure) :
    ...
    im_crop = im.crop((204, 220, 820, 807))
...
variable = 'CoastalRecession'
trim_figure = '/home/jservon/Chapter12_IPCC/figs/Figure_12.7/panel_b_'+region_dict['name']+'_'+variable+'_map_'+scenario+'_'+horizon+'.png'
if not os.path.isdir(os.path.dirname(trim_figure)):
    os.makedirs(os.path.dirname(trim_figure))
