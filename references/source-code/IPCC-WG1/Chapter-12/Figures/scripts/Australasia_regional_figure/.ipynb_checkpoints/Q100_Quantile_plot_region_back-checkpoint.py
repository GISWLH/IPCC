...
# Source: Chapter-12/Figures/scripts/Australasia_regional_figure/.ipynb_checkpoints/Q100_Quantile_plot_region_back-checkpoint.ipynb
...
for (continent in names(regions_by_continent)){
    ...
    metrics_filename = paste('/home/jservon/Chapter12_IPCC/data/Figure_12.7/Q100_',ensemble,'/Q100_',wclim_period,'.nc_',wcontinent,'.txt',sep='')
...
for (continent in names(regions_by_continent)){
    ...
    metrics_filename = paste('/home/jservon/Chapter12_IPCC/data/Figure_12.7/Q100_',wensemble,'/Q100_',wclim_period,'.nc_',wcontinent,'.txt',sep='')
    print(paste('Raw values :', metrics_filename))
    res_metrics_by_AR6region = metrics_reader(metrics_filename)
    for (region_name in names(res_metrics_by_AR6region)){
        all_regions[[region_name]][[ensemble]][[clim_period]][['vals']] = res_metrics_by_AR6region[[region_name]]
...
"barplot_satellite_chap12"=function(data, title, ylabel, do_xlab='TRUE',
    ...
    for (yval in vert_scale(seq(-0.2,0.8,by=0.2), ylim_diff)){
...
figname = paste('/home/jservon/Chapter12_IPCC/figs/Figure_12.7/',region,'_Q100_boxplot.png',sep='')
...
layout(mat, heights=c(1,1.2,0.7), widths=c(1.15,1,1.15))
i = 1
continent_regions = c('NAU', 'CAU', 'EAU', 'SAU', 'NZ')
...
draw_legend_box_hor(all_regions[[subregion]], mar=c(0,1,3,0))
dev.off()
