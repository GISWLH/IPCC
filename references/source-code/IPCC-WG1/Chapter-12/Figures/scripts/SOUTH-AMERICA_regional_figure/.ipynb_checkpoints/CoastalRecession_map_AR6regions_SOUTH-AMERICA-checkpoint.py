...
# Source: Chapter-12/Figures/scripts/SOUTH-AMERICA_regional_figure/.ipynb_checkpoints/CoastalRecession_map_AR6regions_SOUTH-AMERICA-checkpoint.ipynb
...
def retrieve_AR6regions_for_region(region_name):
    ...
    with open(regions_filename) as csvfile:
        ...
        for row in spamreader:
            if row[3] in ['NWS','NSA','SAM','NES','SWS','SES','SSA','SCA']:
                region_dict = dict()
                lats_vect = []
                lons_vect = []
...
def add_AR6regions(wks, base_map, regions):
    ...
    adjp = dict(
        SCA = dict(lon=-5, lat=-5),
        NSA = dict(lon=-3, lat=-3),
        SAM = dict(lon=2, lat=1),
        NES = dict(lon=-1, lat=0),
        SWS = dict(lon=-3, lat=0),
        SES = dict(lon=-2, lat=2),
        SSA = dict(lon=8, lat=0),
    )
...
scenario = 'RCP85'
horizon = '2100'
region_dict = dict(name='SOUTH-AMERICA',
                    lonmin = -109,
                    lonmax = -30,
                    latmin = -60,
                    latmax = 27,
...
# Draw a text string labeling the marker
Ngl.text_ndc(wks,"Shoreline position change by "+horizon,0.5,0.85,txres)
Ngl.text_ndc(wks,"CMIP5 RCP8.5",0.5,0.82,txres)
...
trim_figure = CWD+'/../../figs/Figure_12.8/panel_b_SOUTH-AMERICA_'+variable+'_map_'+scenario+'_'+horizon+'.pdf'
...
def extract_plot(figure_file,trim_figure) :
    ...
    im_crop = im.crop((225, 140, 792, 900))
...
variable = 'CoastalRecession'
trim_figure = CWD+'/../../figs/Figure_12.8/SOUTH-AMERICA_'+variable+'_map_'+scenario+'_'+horizon+'.png'
