...
# Source: Chapter-12/Figures/scripts/SOUTH-AMERICA_regional_figure/.ipynb_checkpoints/Q100_Quantile_plot_region_back-checkpoint.ipynb
...
# %% cell 6
# -- List of regions for SOUTH-AMERICA
continent_regions = c('SCA','NWS','NSA','SAM','NES','SWS','SES','SSA')

regions_by_continent = list(
    CAM=c('SCA'),
    SAM=c('NWS','NSA','SAM','NES','SWS','SES','SSA')
)
...
for (continent in names(regions_by_continent)){
    ...
    metrics_filename = paste('/home/jservon/Chapter12_IPCC/data/Figure_12.8/Q100_',ensemble,'/Q100_',wclim_period,'.nc_',wcontinent,'.txt',sep='')
...
for (continent in names(regions_by_continent)){
    ...
    metrics_filename = paste('/home/jservon/Chapter12_IPCC/data/Figure_12.8/Q100_',wensemble,'/Q100_',wclim_period,'.nc_',wcontinent,'.txt',sep='')
    print(paste('Raw values :', metrics_filename))
    res_metrics_by_AR6region = metrics_reader(metrics_filename)
    for (region_name in names(res_metrics_by_AR6region)){
        all_regions[[region_name]][[ensemble]][[clim_period]][['vals']] = res_metrics_by_AR6region[[region_name]]
...
"barplot_satellite_chap12"=function(data, title, ylabel, do_xlab='TRUE',
    ...
    for (yval in vert_scale(c(-0.2,0,0.2,0.4,0.6,0.8), ylim_diff)){
    ...
    for (horizon in c('modern_raw',GWLs, scenarios)){
        ...
        for (ensemble in ensembles){
            ...
            if (horizon=='modern_raw'){
                ...
                if (dumdat[1]<ylim_diff[1]){dumdat[1]=ylim_diff[1]}
                if (dumdat[3]>ylim_diff[2]){dumdat[3]=ylim_diff[2]}
                dat = vert_scale(dumdat, ylim_diff)
            }
            print(dat)
    ...
    # -- Vertical lines
    vline = ylim
    vline[1] = ylim[1] - (ylim[2]-ylim[1])*0.32
    if (do_xlab=='TRUE'){
    ...
    if (do_xlab=='TRUE'){
        cex_text = 1.8
        bline = ylim[1] - 0.36*(ylim[2]-ylim[1])
        #
        # -- Labels GWLs
        GWLs_range = c(1.5,4.35)
        btext = ylim[1] - 0.44*(ylim[2]-ylim[1])
        ...
        text(0.8,btext2,"r.past.", cex=cex_text, font=1, srt=90, adj=c(0.5,0.5))
        ...
        text(mean(time_slices),btext,"Time slices", font=1, cex=cex_text*0.8)
    }#end if do_ylab
...
figname = paste('/home/jservon/Chapter12_IPCC/figs/Figure_12.8/',region,'_Q100_boxplot.png',sep='')
png(figname, width=900,height=1400*0.8)
continent_regions = c('SCA','NSA','NWS','SAM','NES','SWS','SSA','SES')
mat=rbind(
    c(1,2,0),
    c(3,4,5),
    c(6,7,8),
...
layout(mat, heights=c(1,1,1.25,0.7), widths=c(1.15,1,1.15))
i = 1
for (subregion in continent_regions){
    mar = c(1,1,2.5,1)
    if (subregion %in% c('SCA','NWS','SWS')){
        do_ylab_raw='TRUE'
        mar[2]=4
    }else{
        do_ylab_raw='FALSE'
    }#
    if (subregion %in% c('NES','SES')){
        do_ylab_diff='TRUE'
        mar[4]=4
    }else{
        do_ylab_diff='FALSE'
    }#
    if (subregion %in% c('SWS','SSA','SES')){
        do_xlab='TRUE'
