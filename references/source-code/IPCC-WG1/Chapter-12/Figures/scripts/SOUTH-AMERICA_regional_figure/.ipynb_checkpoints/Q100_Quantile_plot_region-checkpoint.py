...
# Source: Chapter-12/Figures/scripts/SOUTH-AMERICA_regional_figure/.ipynb_checkpoints/Q100_Quantile_plot_region-checkpoint.ipynb
...
for (continent in names(regions_by_continent)){
    ...
    metrics_filename = paste(CWD,'/../../data/Figure_12.8/Q100_',ensemble,'/Q100_',wclim_period,'.nc_',wcontinent,'.txt',sep='')
...
for (continent in names(regions_by_continent)){
    ...
    metrics_filename = paste(CWD,'/../../data/Figure_12.8/Q100_',wensemble,'/Q100_',wclim_period,'.nc_',wcontinent,'.txt',sep='')
    print(paste('Raw values :', metrics_filename))
    res_metrics_by_AR6region = metrics_reader(metrics_filename)
    for (region_name in names(res_metrics_by_AR6region)){
        all_regions[[region_name]][[ensemble]][[clim_period]][['vals']] = res_metrics_by_AR6region[[region_name]]
...
"barplot_satellite_chap12"=function(data, title, ylabel, ylims, do_xlab='TRUE', 
    ...
    if (do_xlab=='TRUE'){
        ...
        text(1,btext2-0.06,"r.past.", cex=cex_text, font=1, srt=90, adj=c(0.5,0.5))
...
figname = paste(CWD,'/../../figs/Figure_12.8/panel_c_',region,'_Q100_boxplot.png',sep='')
png(figname, width=900,height=1600)
continent_regions = c('SCA','NSA','NWS','SAM','SWS','NES','SSA','SES')
mat = t(matrix(1:10,2,5))
mat[5,] = 9
#mat[5,2] = 0
layout(mat, heights=c(1,1,1,1.35,0.7), widths=c(1,1))
i = 1
for (subregion in continent_regions){
    mar = c(1,1,2.5,1)
    if (subregion %in% c('SCA','NWS','SWS','SSA')){
        do_ylab_raw='TRUE'
        mar[2]=4
    }else{
        do_ylab_raw='FALSE'
    }#
    if (subregion %in% c('NSA','SAM','NES','SES')){
        do_ylab_diff='TRUE'
        mar[4]=4
    }else{
        do_ylab_diff='FALSE'
    }#
    if (subregion %in% c('SSA','SES')){
        do_xlab='TRUE'
...
# %% cell 25
figname = paste(CWD,'/../../figs/Figure_12.8/panel_c_',region,'_Q100_boxplot.pdf',sep='')
