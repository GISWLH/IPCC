...
# Source: Chapter-12/Figures/scripts/SOUTH-AMERICA_regional_figure/.ipynb_checkpoints/Barplots_coastalrecession-checkpoint.ipynb
...
subregions = c('SCA','NSA','NWS','NES','SWS','SES','SSA')
...
figname = paste(CWD,'/../../figs/Figure_12.8/panel_d_',region,'_CoastalRecession_boxplot_RCP85_2100.png',sep='')
png(figname, width=900,height=1600)
mat = t(matrix(1:10,2,5))
mat[4,2] = 0
mat[5,] = 8
layout(mat, heights=c(1,1,1,1.25,0.7), widths=c(1,1))
i = 1
for (subregion in names(res[[1]])){
    ...
    if (subregion %in% c('SES','SSA')){
        do_xlab='TRUE'
        #mar[1] = 5 # -- bottom margin
    }else{
        do_xlab='FALSE'
    }#
    if (subregion %in% c('SSA')){mar[1] = 5}
...
# %% cell 7
figname = paste(CWD,'/../../figs/Figure_12.8/panel_d_',region,'_CoastalRecession_boxplot_RCP85_2100.pdf',sep='')
