...
# Source: Chapter-12/Figures/scripts/NORTH-AMERICA_regional_figure/.ipynb_checkpoints/CoastalRecession_map_AR6regions_NORTH-AMERICA-checkpoint.ipynb
...
def retrieve_AR6regions_for_region(region_name):
    ...
    with open(regions_filename) as csvfile:
        ...
        for row in spamreader:
            if row[3] in ['NWN','NEN','WNA','CNA','ENA','NCA','CAR']:
                region_dict = dict()
                lats_vect = []
                lons_vect = []
...
def add_AR6regions(wks, base_map, regions):
    ...
    adjp = dict(
        ENA = dict(lon=-3, lat=6),
        CNA = dict(lon=0, lat=2),
        CAR = dict(lon=-7, lat=-6)
    )
...
# Draw a text string labeling the marker
Ngl.text_ndc(wks,"Coastal recession by "+horizon,0.5,0.75,txres)
Ngl.text_ndc(wks,"CMIP5 "+scenario,0.5,0.72,txres)
...
def extract_plot(figure_file,trim_figure) :
    ...
    im_crop = im.crop((200, 240, 820, 797))
...
trim_figure = '/home/jservon/Chapter12_IPCC/figs/Figure_12.10/panel_b_NORTH-AMERICA_'+variable+'_map_'+scenario+'_'+horizon+'.png'
