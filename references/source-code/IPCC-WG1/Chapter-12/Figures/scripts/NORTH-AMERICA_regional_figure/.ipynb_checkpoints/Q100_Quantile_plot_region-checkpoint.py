...
# Source: Chapter-12/Figures/scripts/NORTH-AMERICA_regional_figure/.ipynb_checkpoints/Q100_Quantile_plot_region-checkpoint.ipynb
...
# %% cell 7
# -- List of regions for NORTH-AMERICA
...
for (continent in names(regions_by_continent)){
    ...
    metrics_filename = paste(CWD,'/../../data/Figure_12.10/Q100_',ensemble,'/Q100_',wclim_period,'.nc_',wcontinent,'.txt',sep='')
...
for (continent in names(regions_by_continent)){
    ...
    metrics_filename = paste(CWD,'/../../data/Figure_12.10/Q100_',wensemble,'/Q100_',wclim_period,'.nc_',wcontinent,'.txt',sep='')
    print(paste('Raw values :', metrics_filename))
    res_metrics_by_AR6region = metrics_reader(metrics_filename)
    for (region_name in names(res_metrics_by_AR6region)){
        all_regions[[region_name]][[ensemble]][[clim_period]][['vals']] = res_metrics_by_AR6region[[region_name]]
...
figname = paste(CWD,'/../../figs/Figure_12.10/panel_c_',region,'_Q100_boxplot.png',sep='')
png(figname, width=900,height=1100)
mat = t(matrix(1:8,2,4))
mat[4,2] = 0
layout(mat, heights=c(1,1,1,1.35), widths=c(1,1))
i = 1
for (subregion in continent_regions){
    mar = c(1,1,2.5,1)
    if (subregion %in% c('NWN','WNA','ENA','CAR')){
        do_ylab_raw='TRUE'
        mar[2]=4
    }else{
        do_ylab_raw='FALSE'
    }#
    if (subregion %in% c('NEN','CNA','NCA','CAR')){
        do_ylab_diff='TRUE'
        #mar[4]=4
    }else{
        do_ylab_diff='FALSE'
    }#
    if (subregion %in% c('NEN','CNA','NCA')){mar[4]=4}
    ...
    if (subregion=='CAR'){
        ylims = list(
            ...
            ylim_diff = c(-1,1),
            ylim_diff_levels = c(-1,-0.5,0,0.5,1)
        )           
            
    }else{
        ylims = list(
            ylim_raw = c(0,1.25),
            ylim_raw_levels = seq(0,1.25,by=0.25),
            ylim_diff = c(-0.2,0.3),
            ylim_diff_levels = c(-0.2,-0.1,0,0.1,0.2,0.3)
        )           
    }
...
# %% cell 25
figname = paste(CWD,'/../../figs/Figure_12.10/panel_c_',region,'_Q100_boxplot.pdf',sep='')
