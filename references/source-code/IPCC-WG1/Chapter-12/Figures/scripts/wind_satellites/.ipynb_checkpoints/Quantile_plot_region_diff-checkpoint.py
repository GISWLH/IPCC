...
# Source: Chapter-12/Figures/scripts/wind_satellites/.ipynb_checkpoints/Quantile_plot_region_diff-checkpoint.ipynb
...
# -- CMIP6
ensemble = 'CMIP6'
json_file = paste(CWD,'/../../data/Figure_S12.5/',ensemble,'_sfcWind_diff_AR6_regional_averages.json',sep='')
...
# %% cell 10
ensemble = 'CMIP6'
json_file = paste(CWD,'/../../data/Figure_S12.5/',ensemble,'_sfcWind_diff-perc-baseline_AR6_regional_averages.json',sep='')
json_data <- fromJSON(paste(readLines(json_file), collapse=""))
...
"barplot_satellite_chap12"=function(data, title, ylabel, do_xlab='TRUE', mar=c(4,3,2.5,1),
    ...
    for (horizon in c(GWLs, scenarios)){
        ...
        for (ensemble in ensembles){
            ...
            if (is.na(dat[1])=='FALSE'){
                if (dat[1]<ylim[1]){
                    text(j-0.5,ylim[1]+(ylim[2]-ylim[1])*0.07*e_inf, round(dat[1],1), cex=1.7, font=1,
                         col=data[[ensemble]][[horizon]][['color']])
                    dat[1] = ylim[1]
                    e_inf = e_inf + 1
                }
                if (dat[3]>ylim[2]){
                    text(j-0.5,ylim[2]-(ylim[2]-ylim[1])*0.07*e_sup, round(dat[3],1), cex=1.7, font=1,
                         col=data[[ensemble]][[horizon]][['color']])
                    dat[3] = ylim[2]
                    e_sup = e_sup + 1
                }
    ...
    if (do_xlab=='TRUE'){
        ...
        xmid = 4.5
        xlate = 6.5
        #text(xmid,btext2, "r.past", font=1, cex=cex_text, srt=90, adj=adj_ts)
...
#legend is the last
fsize = 2000
outfilename = paste(CWD,'/../../figs/Satellite_barplots/Figure_S12.5_wind_satellite.png',sep='')
...
# %% cell 21
fsize = 20*1.4
outfilename = paste(CWD,'/../../figs/Satellite_barplots/Figure_S12.5_wind_satellite.pdf',sep='')
